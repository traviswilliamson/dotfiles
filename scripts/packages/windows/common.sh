...
if ! echo "$packagelist" | grep -q Chocolatey.Chocolatey; then
    ...
    winget install --id Chocolatey.Chocolatey -e --source winget --accept-package-agreements --accept-source-agreements || logerror "Failed to install choco"
    choco upgrade chocolatey || logerror "Failed to upgrade choco"
fi

if ! echo "$packagelist" | grep -q Brave.Brave; then
    anyinstalled=true
    info "Installing Brave"
    winget install --id Brave.Brave -e --source winget --accept-package-agreements --accept-source-agreements || logerror "Failed to install Brave"
fi
...
if ! echo "$packagelist" | grep -q Microsoft.PowerToys; then
    ...
    winget install --id Microsoft.PowerToys -e --source winget --accept-package-agreements --accept-source-agreements || logerror "Failed to install Powertoys"
fi
...
if ! echo "$packagelist" | grep -q Microsoft.VisualStudioCode; then
    ...
    winget install --id Microsoft.VisualStudioCode -e --source winget --accept-package-agreements --accept-source-agreements || logerror "Failed to install VSCode"
fi

if ! echo "$packagelist" | grep -q Neovim.Neovim; then
    anyinstalled=true
    info "Installing Neovim"
    winget install --id Neovim.Neovim -e --accept-package-agreements --accept-source-agreements  || logerror "Failed to install Neovim"
fi
...
if ! echo "$packagelist" | grep -q ajeetdsouza.zoxide; then
    ...
    winget install --id ajeetdsouza.zoxide -e --accept-package-agreements --accept-source-agreements || logerror "Failed to install zoxide"
fi
...
if ! echo "$packagelist" | grep -q Bitwarden.Bitwarden; then
    ...
    winget install --id Bitwarden.Bitwarden -e --accept-package-agreements --accept-source-agreements || logerror "Failed to install Bitwarden"
fi

if ! echo "$packagelist" | grep -q OBSProject.OBSStudio; then
    anyinstalled=true
    info "Installing OBS"
    winget install --id OBSProject.OBSStudio -e --accept-package-agreements --accept-source-agreements || logerror "Failed to install OBS"
fi

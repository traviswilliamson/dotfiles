...
if ! echo "$packagelist" | grep -q Discord.Discord; then
    ...
    winget install --id Discord.Discord -e --accept-package-agreements --accept-source-agreements || error "Failed to install Discord"
fi

if ! echo "$packagelist" | grep -q Valve.Steam; then
    anyinstalled=true
    info "Installing Steam"
    winget install --id Valve.Steam -e --accept-package-agreements --accept-source-agreements || error "Failed to install Steam"
fi

if ! echo "$packagelist" | grep -q ElectronicArts.EADesktop; then
    anyinstalled=true
    info "Installing EA"
    winget install --id ElectronicArts.EADesktop -e --accept-package-agreements --accept-source-agreements || error "Failed to install EA"
fi

if ! echo "$packagelist" | grep -q EpicGames.EpicGamesLauncher; then
    anyinstalled=true
    info "Installing Epic Games"
    winget install --id EpicGames.EpicGamesLauncher -e --accept-package-agreements --accept-source-agreements || error "Failed to install Epic Games"
fi

if ! echo "$packagelist" | grep -q GOG.Galaxy; then
    anyinstalled=true
    info "Installing GOG Galaxy"
    winget install --id GOG.Galaxy -e --accept-package-agreements --accept-source-agreements || error "Failed to install GOG Galaxy"
fi
